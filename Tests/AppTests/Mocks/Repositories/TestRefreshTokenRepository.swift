...
class TestRefreshTokenRepository: RefreshTokenRepository, TestRepository {
    ...
    func existByUserID(_ userID: UUID) -> EventLoopFuture<Bool> {
        let token: RefreshToken? = tokens.first(where: { $0.user.id == userID })
     
        return eventLoop.makeSucceededFuture(token != nil)
    }
